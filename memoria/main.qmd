...
#### Modelo recurrente
Además de las estructuras anteriormente mencionadas, se evaluaron modelos recurrentes. En estos, cada espectro se consideró cada longitud de onda como un paso de la secuencia, y el flujo correspondiente era la variable de entrada. Esta arquitectura permite conservar información de pasos anteriores de la secuencia [@schmidt2019rnn].
...
Finalmente, se evaluaron arquitecturas con mecanismos de atención [@vaswani2017attention], permitiendo que no se resuma el entreno en el último estado recurrente, sino que el modelo conserva información de todos los pasos de la secuencia y asigne pesos a aquellas regiones del espectro que se consideran más importantes para la predicción.  

En la arquitectura de estos dos últimos modelos mencionados, se redujo las épocas de espera dado que la diferencia entre esperar 2 o 4 épocas no reflejaba mejoras en las estadísticas pero sí en el tiempo de entrenamiento.

En todas las arquitecturas probadas, la capa de salida del modelo tenía dimensión igual al número de puntos del espectro real de [SDSS]{.acr}. 
...
La siguiente arquitectura usó una capa Bidireccional después de la de entrada, y a continuación, 4 capas densas con capas de funciones de activación LeakyReLU. 

Las métricas obtenidas tras el entrenamiento de las 6 épocas del modelo fueron los incluidos en la @tbl-BIRNN-metrics :
...
Los ejemplos usados para la representación visual de las aproximaciones, tampoco presentan grandes diferencias, tal y como observamos en las figuras @fig-BIRNN
...
Ejemplos de espectros generado por el modelo de redes neuronales recurrentes con una capa bidireccional.
:::
...
Por último, si nos basamos en los resultados obtenidos de la estimación de características astrofísicas con *iSpec*, las diferencias entre los valores predichos con los espectros de [SDSS]{.acr} reales y los predichos por los modelos son los incluidos en la @tbl-RNN-iSpec:
...
Para comparar la eficacia de cada modelo estudiado con estadísticas añadidas a las ya utilizadas, se cuantificó la diferencia entre los espectros reales de [SDSS]{.acr} y los predichos por cada modelo a partir del cálculo de chi-cuadrado para cada observación en el conjunto de test. 
...
donde $F_{\mathrm{real},i}$ y $F_{\mathrm{pred},i}$ representan, respectivamente, el flujo real y predicho en el punto espectral $i$, respectivamente, mientras que $\mathrm{ivar}_i$ es la varianza inversa sacada de los datos de [SDSS]{.acr}.

Por otro lado, como $\chi^2$ también depende del número de puntos de cada espectro, se consideró una versión normalizada dividida por la cantidad de puntos espectrales válidos (con *ivar* mayor que 0). Al normalizarla, facilita la comparación entre los distintos espectros, dado que valores próximos a la unidad indican que las diferencias entre ambos espectros son comparables a la incertidumbre de la muestra, mientras que valores mayores implican discrepancias superiores a las esperadas en los puntos espectrales. 
Teniendo esto en cuenta, la fórmula final fue:

$$\chi^2_{\mathrm{norm}} = \frac{\chi^2}{N} $$

A partir de la cual se obtuvieron los valores de la @tbl-chi-square:
...
Los resultados obtenidos demuestran que la arquitectura densa normalizada con la mediana de cada espectro es la que mejor comportamiento tiene. Ni los modelos convolucionales ni los recurrentes mejoraron este resultado, aunque la arquitectura recurrente con una capa de GRU se queda muy próxima. 
Por otro lado, el modelo denso normalizado con el máximo de cada espectro es la menos estable tal y como se aprecia en la diferencia entre el mínimo y máximo, dos órdenes de magnitud por encima de la del resto de modelos. Presenta una mediana 4 veces superior al modelo con mejores datos, confirmando que es el peor de los modelos densos y recurrentes destacados en el entrenamiento.
...
La conclusión extraída con el estadístico chi-cuadrado es la misma que la obtenida de las métricas de entrenamiento y test de los modelos. Si bien es cierto, las diferencias entre los modelos densos y recurrentes no son muy grandes, pero los valores más pequeños se encuentran en el modelo denso normalizado con la mediana. 
En cuanto a iSpec, los resultados indican que todos los modelos tienen un comportamiento similar en cuanto a los datos que este aproxima, así que no podemos destacar un modelo en concreto, pero sí afirmar que las predicciones producen datos astrofísicos muy parecidos a los que producen los espectros reales.

Si nos centramos en los resultados obtenidos por la red que mejores estadísticas presenta generalmente, es decir, la red neuronal Densa con la normalización por la mediana, observamos en la @fig-HR-CHI-dense que no destaca una categoría de estrellas donde el modelo haya funcionado mejor.
...
Diagrama HR del estadístico Chi-cuadrado aplicado sobre la muestra de test en el modelo denso.
:::

Aún así, cabe destacar que la cantidad de espectros analizados en la secuencia principal es mucho mayor a los de enanas blancas, así que no disponemos de una muestra suficientemente representativa para asegurar un comportamiento igualitario ante ambos conjuntos.
...
Asimismo, como se observó al analizar los datos a través de un diagrama [HR]{.acr}, las estrellas gigantes no estaban representadas durante el entrenamiento del modelo, con lo que podría extenderse su uso a esta categoría aumentando el conjunto de datos usado.

La programación de los modelos se ha basado en scripts y notebooks de Python, aunque en un futuro podría facilitarse el acceso a este servicio mediante el desarrollo de una aplicación que permita acceder al modelo de una forma mas sencilla. 
