...
Una vez extraído el conjunto de datos inicial, las parejas de espectros deberán pasar una serie de requisitos mínimos de calidad para ser usados en el entrenamiento, por ejemplo se descartarán aquellos que presenten posibles anomalías o estén faltos de variabilidad en el flujo, pudiendo indicar un error en el espectro. Estos descartes de datos se describen en profundidad en la sección 5.
Además, se recortarán los rangos de longitud de onda a aquellos que se sobrepongan entre ambas muestras y que no incluyan anomalías de forma generalizada. 
...
Para analizar la distribución física de los datos usados en el entrenamiento del modelo, se usa una representación del diagrama HR ( Hertzsprung-Russell, ). Este diagrama representa la relación entre la luminosidad de las estrellas (eje Y) y su temperatura o color (eje X).
...
## Evaluación
Durante el entrenamiento del modelo se pudo observar una evolución estable de las métricas controladas, tanto en el conjunto de entrenamiento como el de validación. Vemos esta disminución constante representada en la @fig-loss-evolution, @fig-mse-evolution y @fig-mae-evolution .
...
En la última época llevada a cabo, se obtuvo una pérdida del conjunto de entrenamiento de 0.0084, un error absoluto medio de 0.0726 y un error cuadrático medio de 0.0189.
En el conjunto de validación las métricas obtenidas fueron muy cercanas, siendo un 0.0083 de pérdida, un [MAE]{.acr} de 0.0729 y [MSE]{.acr} de 0.0173. 

Una vez finalizado el entrenamiento del modelo, se evaluó su eficacia sobre el conjunto de test. Los valores obtenidos en este conjunto tuvieron una pérdida de 0.0087, [MAE]{.acr} de 0.0734 y [MSE]{.acr} de 0.0187. Estos datos son cercanos a los obtenidos en la muestra de validación y entrenamiento, reforzando la idea de que el modelo tiene un comportamiento estable ante los datos que no han sido vistos anteriormente. 
