...
Para ser exactos, trataremos de relacionar los espectros de baja resolución XP de Gaia DR3 (constituidos por los espectros azul y rojo) y los espectros ópticos de [SDSS]{.acr} DR13.
...
En la @fig-espectro-ejemplo se muestra un ejemplo de espectro estelar observado por [SDSS]{.acr}, donde se pueden apreciar las líneas de absorción y emisión mencionadas.

![Ejemplo de espectro estelar observado por [SDSS]{.acr}](_partials/assets/espectro_sloan_1.png){#fig-espectro-ejemplo}
...
A diferencia de Gaia, [SDSS]{.acr} proporciona directamente espectros ópticos en longitud de onda y flujo, y estos datos se almacenan distribuidos a lo largo de archivos FITS. En esta base se pueden encontrar variables como la longitud de onda y el flujo espectral.

Dentro del proyecto de [SDSS]{.acr}, el programa MaNGA (*Mapping Nearby Galaxies at Apache Point Observatory*) se encarga de procesar los datos con tal de crear el mapa estelar, en su artículo original para [SDSS]{.acr}-IV [@manga2015] encontramos que el rango espectral aproximado de [SDSS]{.acr} se mantiene entre los 360 y 1030 nm y su resolución se encuentra en R $\sim$ 2000.
...
![Ejemplo aleatorio del espectro captado de un mismo cuerpo por Gaia y [SDSS]{.acr}](_partials/assets/espectro_gaia_sloan.png){#fig-gaia-sdss}

Podemos observar como claramente el intervalo que [SDSS]{.acr} cubre es muy similar a Gaia, pero existe una gran diferencia en cuanto a la suavidad de las curvas representadas. Como vemos, [SDSS]{.acr} incluye muchos más detalles que Gaia, su espectro tiene una calidad superior. 
...
La integración y estandarización de catálogos astronómicos masivos representa un desafío técnico fundamental, especialmente al trabajar con fuentes de naturalezas distintas como Gaia y [SDSS]{.acr}. Mientras que Gaia provee un volumen de datos astrométricos, fotométricos y espectroscópicos sin precedentes, sus espectros BP/RP operan en resoluciones bajas y requieren complejos procesos de calibración para mitigar el ruido espacial y las fluctuaciones. Cabe matizar que el espectrómetro RVS de Gaia alcanza una resolución incluso mayor que la de [SDSS]{.acr}, pero al requerir estrellas más brillantes dispone de muchas menos fuentes observadas. En contraste, [SDSS]{.acr} aporta espectros ópticos de mayor resolución que los BP/RP. La necesidad de interconectar ambas misiones define el núcleo de este estudio: establecer una relación sólida capaz de traducir y equiparar los espectros de baja resolución de Gaia con los espectros de mayor resolución de [SDSS]{.acr}.
...
Puesto que en la preselección de las observaciones se han tenido en cuenta criterios de calidad, y Gaia DR3 ofrece datos ya procesados y relaciones con [SDSS]{.acr}, el preprocesado en este caso consiste en la reducción del ruido, mejorando así la relación señal-ruido (*Signal-to-Noise Ratio*, SNR), aplicada a las observaciones de [SDSS]{.acr}, que tienen más variabilidad y pueden así aumentar la complejidad del entrenamiento.
...
El conjunto de datos usado para el entrenamiento del modelo debe estar formada por pares de espectros de Gaia y [SDSS]{.acr} que correspondan al mismo objeto astronómico. Para garantizar esta correspondencia se debe usar el catálogo de cruce de datos que Gaia incluye en su tercera liberación de datos.
...
También tuvimos en cuenta la relación de escala entre los espectros de Gaia y [SDSS]{.acr}. En algunos casos observábamos que los pares diferían exageradamente en la escala usada, por lo que con el mismo criterio de los percentiles nombrados anteriormente, los casos más extremos fueron eliminados. De esta forma mantuvimos los pares en los que la relación entre la entrada y la salida fuese coherente.

Para obtener un correcto funcionamiento del modelo, tuvimos que reducir la dimensionalidad del espectro de [SDSS]{.acr} mediante una agrupación de bines consecutivos. [SDSS]{.acr} contaba inicialmente con 3600 observaciones por espectro, mientras que Gaia tan solo 343. Esta diferencia suponía una dificultad añadida para el modelo, así que aplicamos una función de agrupación, la cual sustituía por un único valor promedio cada bloque de n valores consecutivos del flujo de [SDSS]{.acr}. Tras valorar los beneficios y desventajas de esta agrupación, se consideró una agrupación en conjuntos de 7 bines, reduciendo la dimensión de [SDSS]{.acr} a 380 puntos. Este valor suponía el equilibrio entre la pérdida de información debido a la reducción de puntos y la mejora del problema de regresión del modelo.  
...
![Ejemplo 1 del espectro generado por el modelo y los de Gaia y [SDSS]{.acr} originales](_partials/assets/espectro_generado_123.png){#fig-generado_123}

![Ejemplo 2 del espectro generado por el modelo y los de Gaia y [SDSS]{.acr} originales](_partials/assets/espectro_generado_178.png){#fig-generado_178}

En ellas observamos tres espectros. Por un lado, en azul encontramos el espectro que el modelo pretende predecir, los puntos de [SDSS]{.acr} una vez han sido recortados y agrupados. Por otro, en verde podemos ver la entrada que recibe el modelo, siendo esta el espectro calibrado de Gaia. Los puntos de Gaia han sido escalados a la misma unidad que [SDSS]{.acr} para poder representar ambos espectros en la misma escala, pero en los datos originales, se usan escalas diferentes. 
...
En resumen, podemos concluir que este trabajo demuestra la viabilidad de diseñar un modelo de redes neuronales capaz de aproximar espectros captados por Gaia a una representación similar a los observados por [SDSS]{.acr}. Aunque los espectros generados tienen menor nivel de detalle que los espectros originales de [SDSS]{.acr}, si presentan una mayor resolución que los datos de entrada de Gaia. Este hecho permite dejar una puerta abierta a varias líneas de mejora, las cuales serán comentadas en el siguiente apartado. 
