...
Una vez extraído el conjunto de datos inicial, las parejas de espectros deberán pasar una serie de requisitos mínimos de calidad para ser usados en el entrenamiento, por ejemplo se descartarán aquellos que presenten posibles anomalías o estén faltos de variabilidad en el flujo, pudiendo indicar un error en el espectro. Además, se recortarán los rangos de longitud de onda a aquellos que se sobrepongan entre ambas muestras y que no incluyan anomalías de forma generalizada. 
...
Finalmente, aplicamos limitaciones en cuanto a la calidad de los espectros. A partir de la relación señal-ruido de cada observación, descartamos aquellas con los valores de [SNR]{.acr} más bajos con la intención de impedir el uso de parejas de espectros con [SNR]{.acr} muy diferentes. Para hacerlo, descartamos los valores mas extremos a partir del percentil 95.
...
Probamos también la adición de los datos de la relación señal-ruido que Gaia contiene en su catálogo como parte del conjunto de entrada. En este caso, los resultados obtenidos no mejoraron significativamente, por lo que se continuó el estudio sin este dato de entrada.
...
En esta *a* representa el error entre el valor real y pedicho, y $\delta$ define el valor frontera en el que la función deja de estar definida como [MSE]{.acr} y pasa a ser mas similar a [MAE]{.acr}.
...
## Evaluación
Durante el entrenamiento del modelo se pudo observar una evolución estable de las métricas controladas, tanto en el conjunto de entrenamiento como el de validación. Vemos esta disminución constante representada en la @fig-loss_evolution, @fig-mse-evolution y @fig-mae-evolution .
...
En las gráficas detalladas de la @fig-generado_123 y la @fig-generado_191, se muestran dos ejemplos distintos donde se observan superpuestos los datos de entrada del modelo, es decir el espectro captado por Gaia, el espectro de SDSS real y el generado a partir del modelo entrenado. 

![Ejemplo 1 del espectro generado por el modelo y los de Gaia y SDSS originales](_partials/assets/espectro_predicho_123_full.png){#fig-generado_123}

![Ejemplo 2 del espectro generado por el modelo y los de Gaia y SDSS originales](_partials/assets/eespectro_predicho_191_full.png){#fig-generado_191}
