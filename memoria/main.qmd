...
Para analizar la distribución física de los datos usados en el entrenamiento del modelo, se usó una representación del diagrama HR ( Hertzsprung-Russell, ). Este diagrama representa la relación entre la luminosidad de las estrellas (eje Y) y su temperatura o color (eje X).

En concreto, para estudiar los cuerpos celestes usados en este ttrabajo, se usó la fuente de datos de Gaia presente en la base de datos usada en el modelo de redes neuronales que comentaremos en el siguiente apartado. En el eje horizontal se considera la variable bp_rp, que corresponde con el índice de color obtenido a partir de la diferencia entre las medias de las magnitudes de BP y RP. Cuanto menor sea este valor, más azul y caliente sería la estrella, mientras que los valores mayores representan estrellas rojas y frías.

En el eje vertical encontraremmos la vairable phot_g_mean_mag, la magnitud aparente media en la banda G, que nos indica lo brillante que observamos la estrella a partir de las observaciones realizadas. Este valor depende de variables como por ejemplo la distancia, por lo que para convertir todos los valores a la misma escala y obtener la magnitud absoluta de cada observación, se usó el paralaje de cada una. 
Tras esta modificación, aquellas estrellas mas luminosas, presentarán valores negativos, mientras que las más luminosas se mantienen en valores mayores. 
...
En este apreciamos como el conjunto de datos no está formado por un único tipo de estrella, sino que encontramos objetos con propiedades físicas diferentes. Si bien es cierto, encontramos un grupo mayoritario de observaciones tal y como se aprecia en la parte superior izquierda del gráfico. 
...
Al mismo tiempo, se observan algunos datos mas dispersos y distantes de las regiones mas ocupadas, los cuales podrían considerarse outliers para el modelo a entrenar.
...
Finalmente, aplicamos limitaciones en cuanto a la calidad de los espectros. A partir de la relación señal-ruido de cada observación, descartamos aquellas con los valores de [SNR]{.acr} más bajos con la intención de impedir el uso de parejas de espectros con [SNR]{.acr} muy diferentes. 
...
En el conjunto de validación las métricas obtenidas fueron muy cercanas, siendo un 0.0074 de pérdida, un [MAE]{.acr} de 0.0676 y [MSE]{.acr} de 0.0152. 

Una vez finalizado el entrenamiento del modelo, se evaluó su eficacia sobre el conjunto de test. Los valores obtenidos en este conjunto tuvieron una pérdida de 0.0081, [MAE]{.acr} de 0.0697 y [MSE]{.acr} de 0.0168. Estos datos son cercanos a los obtenidos en la muestra de validación y entrenamiento, reforzando la idea de que el modelo tiene un comportamiento estable ante los datos que no han sido vistos anteriormente. 
