...
Uso:
    uv run python -m pipeline.parse_log
    uv run python -m pipeline.parse_log --log data/pipeline.log
    uv run python -m pipeline.parse_log --run -1   # última ejecución (por omisión)
    uv run python -m pipeline.parse_log --run 0    # primera ejecución
"""
...
def _suggest_plate_end(run: dict, target: int) -> None:
    ...
    print("\n".join([
      ...
      , "\n  Comando sugerido:"
      , f"  uv run python -m pipeline.run --force "
