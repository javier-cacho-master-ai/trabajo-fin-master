"""
Persistencia del entrenamiento de los modelos convolucionales.

Cada entrenamiento deja cuatro ficheros en la carpeta del modelo, uno por cada
forma de dato, de modo que las celdas de evaluación y análisis puedan ejecutarse
en una sesión nueva sin repetir el entrenamiento:
...
- `<modelo>_test_data.npz`: las variables del conjunto de test que consumen las
  celdas posteriores. Son arrays homogéneos de hasta decenas de megabytes, para
...
Los arrays que se pueden regenerar en un paso determinista —las predicciones del
modelo, sin ir más lejos— no se guardan: se obtienen del modelo y de este mismo
`.npz`.
"""
...
def load_checkpoint(model_path, history_path, summary_path):
    """
